...
db.exec(`
  CREATE TABLE IF NOT EXISTS goals (
    ...
    keywords TEXT NOT NULL DEFAULT '[]', status TEXT NOT NULL DEFAULT 'active',
    origin TEXT NOT NULL DEFAULT 'manual', sort_order INTEGER,
    item_count INTEGER DEFAULT 0, source_count INTEGER DEFAULT 0,
    created_at TEXT DEFAULT (datetime('now')), updated_at TEXT DEFAULT (datetime('now'))
  );
  CREATE TABLE IF NOT EXISTS projects (
    id TEXT PRIMARY KEY, goal_id TEXT REFERENCES goals(id),
    name TEXT NOT NULL, source TEXT NOT NULL, source_id TEXT,
    status TEXT DEFAULT 'active', metadata TEXT, created_at TEXT DEFAULT (datetime('now'))
  );
  CREATE TABLE IF NOT EXISTS work_items (
    id TEXT PRIMARY KEY, source TEXT NOT NULL, source_id TEXT NOT NULL,
    item_type TEXT NOT NULL, title TEXT NOT NULL, body TEXT, author TEXT,
    status TEXT, priority TEXT, url TEXT, metadata TEXT,
  ...
  CREATE TABLE IF NOT EXISTS item_tags (
    item_id TEXT REFERENCES work_items(id), tag_id TEXT REFERENCES tags(id),
    confidence REAL DEFAULT 1.0, PRIMARY KEY (item_id, tag_id)
  );
  CREATE TABLE IF NOT EXISTS links (
    id TEXT PRIMARY KEY, source_item_id TEXT REFERENCES work_items(id),
    target_item_id TEXT REFERENCES work_items(id),
    link_type TEXT NOT NULL, confidence REAL DEFAULT 1.0,
    created_at TEXT DEFAULT (datetime('now'))
  );
  CREATE TABLE IF NOT EXISTS metrics_snapshots (
    id TEXT PRIMARY KEY, goal_id TEXT REFERENCES goals(id),
    snapshot_date TEXT NOT NULL, total_items INTEGER, done_items INTEGER,
    active_items INTEGER, stale_items INTEGER, velocity_7d REAL,
    avg_cycle_time_days REAL, cross_ref_count INTEGER, metadata TEXT,
    UNIQUE(goal_id, snapshot_date)
  );
  CREATE TABLE IF NOT EXISTS sync_log (
    id TEXT PRIMARY KEY, source TEXT NOT NULL, started_at TEXT NOT NULL,
    completed_at TEXT, items_synced INTEGER DEFAULT 0,
...
const goals = [
  ...
  { id: 'platform', name: 'Platform Modernization', description: 'Django→microservices, Celery→Lambda, bundle size, infra', keywords: ['django','microservice','celery','lambda','bundle','infra','migration','platform','ottiapi','api gateway','terraform','devops','ci','cd','deploy','docker','ecs','architecture','tdd process','paas'], sort_order: 2 },
...
for (const g of goals) {
  insertGoal.run(g.id, g.name, g.description, JSON.stringify(g.keywords), 'active', 'inferred', g.sort_order);
}
console.log(`Seeded ${goals.length} goals`);

// ── Insert helper ──
const insertItem = db.prepare(`
  INSERT OR IGNORE INTO work_items (id, source, source_id, item_type, title, body, author, status, priority, url, metadata, created_at, updated_at)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`);
...
try {
  ...
  for (const issue of jira) {
    insertItem.run(
      uuid(), 'jira', issue.key, issue.issueType || 'task',
      issue.summary, null, issue.assignee || null,
      normalizeStatus(issue.status), issue.priority || null,
      issue.url, JSON.stringify({ labels: issue.labels || [] }),
      parseDate(issue.created), parseDate(issue.updated)
    );
    count++;
  }
...
try {
  ...
  for (const msg of slack) {
    insertItem.run(
      uuid(), 'slack', `slack-${uuid().slice(0,8)}`, 'message',
      msg.text.slice(0, 200), msg.text, msg.author || null,
      'done', null, msg.url || null,
      JSON.stringify({ channel: msg.channel }),
      parseDate(msg.date), parseDate(msg.date)
    );
    count++;
  }
...
try {
  ...
  for (const m of meetings) {
    insertItem.run(
      uuid(), 'granola', m.id, 'meeting',
      m.title, m.summary || null, m.participants?.[0] || null,
      'done', null, m.url || null,
      JSON.stringify({ participants: m.participants || [] }),
      parseDate(m.date), parseDate(m.date)
    );
    count++;
  }
...
try {
  ...
  for (const page of notion) {
    insertItem.run(
      uuid(), 'notion', page.id, page.type || 'page',
      page.title, null, null,
      'active', null, page.url || null,
      null, parseDate(page.date), parseDate(page.date)
    );
    count++;
  }
...
console.log(`Classification: ${classified} item-goal associations created`);
...
for (const goal of activeGoals) {
  const counts = db.prepare(`
    ...
    FROM item_tags it JOIN work_items wi ON wi.id = it.item_id
    WHERE it.tag_id = ?
  `).get(goal.id);

  const linkCnt = db.prepare(`
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(uuid(), goal.id, today, counts.total||0, counts.done||0, counts.active||0, counts.stale||0, 0, linkCnt.c||0);
}
...
for (const src of ['jira', 'slack', 'granola', 'notion']) {
  ...
  db.prepare('INSERT INTO sync_log (id, source, started_at, completed_at, items_synced, status) VALUES (?, ?, ?, ?, ?, ?)').run(
    uuid(), src, now, now, cnt.c, 'complete'
  );
}

// ── Summary ──
console.log('\n═══ Database Summary ═══');
const totalItems = db.prepare('SELECT COUNT(*) as c FROM work_items').get();
const totalLinks = db.prepare('SELECT COUNT(*) as c FROM links').get();
const totalTags = db.prepare('SELECT COUNT(*) as c FROM item_tags').get();
console.log(`Total work items: ${totalItems.c}`);
console.log(`Total cross-refs: ${totalLinks.c}`);
console.log(`Total goal associations: ${totalTags.c}`);
...
const bySrc = db.prepare('SELECT source, COUNT(*) as c FROM work_items GROUP BY source').all();
console.log('\nBy source:');
for (const s of bySrc) console.log(`  ${s.source}: ${s.c}`);

db.close();
console.log('\nDone! Database populated at:', DB_PATH);
