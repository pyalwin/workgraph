...
export function VolumeChart({ data, splitDate }: VolumeChartProps) {
  ...
  return (
    <div className="bg-surface border border-black/[0.07] rounded-card p-[22px]">
      <div className="text-[0.67rem] font-semibold uppercase tracking-[0.07em] text-g5 mb-[18px]">Daily Volume</div>
      <div className="flex items-end gap-[3px] h-[160px] pt-[10px]">
        {data.map((d) => {
          const heightPx = Math.max(Math.round((d.count / maxCount) * 140), d.count > 0 ? 6 : 2);
          const isPeak = d.count === peakCount && d.count > 0;
          const isSplit = splitDate && d.date === splitDate;
          return (
            <div key={d.date} className="flex-1 flex flex-col items-center justify-end gap-1 relative" style={{ height: 150 }}>
              {isSplit && (
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[2px] h-full bg-accent-red/40 z-10" />
              )}
              <div
              ...
              />
              {data.length <= 14 && (
                <span className="text-[0.55rem] text-g5 tabular-nums leading-none">
                  {d.date.slice(5)}
                </span>
              )}
            </div>
