...
export const WORKSPACE_PRESETS: WorkspacePreset[] = [
  ...
  },
  {
    id: 'otti',
    name: 'Otti Assistant',
    workflow: 'Session telemetry -> adoption metrics -> deployment comparison -> user insights',
    connectors: ['Session telemetry', 'Identity metadata', 'Deployment markers'],
  },
];

export function WorkspaceOnboarding() {
  const router = useRouter();
  const { setState, workspaces, refreshWorkspaces } = useWorkgraphState();
  ...
  const create = async () => {
    setSaving(true);
    try {
      const existingOtti = selected === 'otti' ? workspaces.find((workspace) => workspace.id === 'otti') : null;
      if (existingOtti) {
        const res = await fetch('/api/workspaces/otti', {
          method: 'PATCH',
